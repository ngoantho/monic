...
#define MONIC_TAB_STOP 8
#define MONIC_QUIT_TIMES 3
...
int editorRowCxToRx(erow *row, int cx) {
  ...
  for (j = 0; j < cx; j++) {
    if (row->chars[j] == '\t')
      rx += (MONIC_TAB_STOP - 1) - (rx % MONIC_TAB_STOP);
    rx++;
  }
  return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
  ...
  for (cx = 0; cx < row->size; cx++) {
    if (row->chars[cx] == '\t')
      cur_rx += (MONIC_TAB_STOP - 1) - (cur_rx % MONIC_TAB_STOP);
    cur_rx++;
    if (cur_rx > rx)
      return cx;
  }
...
void editorUpdateRow(erow *row) {
  ...
  for (j = 0; j < row->size; j++)
    if (row->chars[j] == '\t')
      tabs++;
  free(row->render);
  row->render = malloc(row->size + tabs * (MONIC_TAB_STOP - 1) + 1);
  int idx = 0;
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
      while (idx % MONIC_TAB_STOP != 0)
        row->render[idx++] = ' ';
    } else {
...
void editorProcessKeypress() {
  static int quit_times = MONIC_QUIT_TIMES;
